...
func TestSegments(t *testing.T) {
	for _, tc := range []struct {
		...
		{"one command", "cat a b",
			[]Segment{{[]string{"cat", "a", "b"}, nil, nil, true, "", false, 0}}},
		...
		{"a redirect belongs to the segment it was written in",
			...
			[]Segment{
				{[]string{"cd", "/tmp"}, nil, nil, true, "", false, 0},
				{[]string{"cat", "/dev/null"}, []string{"evil"}, nil, true, "&&", false, 1},
			}},
		...
		{"a pipeline's stages share a number", "a | b | c",
			[]Segment{
				{[]string{"a"}, nil, nil, false, "", false, 0},
				{[]string{"b"}, nil, nil, false, "", false, 0},
				{[]string{"c"}, nil, nil, false, "", false, 0},
			}},
		...
		{"an assignment at the top level persists", "f=x; cat $f",
			[]Segment{
				{[]string{"f=x"}, nil, nil, true, "", false, 0},
				{[]string{"cat", "$f"}, nil, nil, true, "", false, 1},
			}},
		{"one in a subshell does not", "(f=x); cat $f",
			[]Segment{
				{[]string{"f=x"}, nil, nil, false, "", false, 1},
				{[]string{"cat", "$f"}, nil, nil, true, "", false, 3},
			}},
		{"a backgrounded one does not", "f=x & cat $f",
			[]Segment{
				{[]string{"f=x"}, nil, nil, false, "", false, 0},
				{[]string{"cat", "$f"}, nil, nil, true, "", false, 1},
			}},
		...
		{"an fd number before a redirect is not an operand", "cat x 2> err",
			[]Segment{{[]string{"cat", "x"}, []string{"err"}, nil, true, "", false, 0}}},
		// A duplication target is an fd or `-`, not a path.
		{"a dup target is not a path", "cat x 2>&1",
			[]Segment{{[]string{"cat", "x"}, nil, nil, true, "", false, 0}}},
		{"nor a close", "cat x 2>&-",
			[]Segment{{[]string{"cat", "x"}, nil, nil, true, "", false, 0}}},
		...
		{"a dup operator with a filename target does name a path", "cat x >&out",
			[]Segment{{[]string{"cat", "x"}, []string{"out"}, nil, true, "", false, 0}}},
		...
		{"a file named 2 before a redirect is lost, as upstream loses it",
			"cat 2 > out",
			[]Segment{{[]string{"cat"}, []string{"out"}, nil, true, "", false, 0}}},
		{"every target of a multiply-redirected command", "sort <in >out 2>err",
			[]Segment{{[]string{"sort"}, []string{"in", "out", "err"}, []string{"in"}, true, "", false, 0}}},
		{"the three-character redirect operators", "a &>> both",
			[]Segment{{[]string{"a"}, []string{"both"}, nil, true, "", false, 0}}},
		{"a clobbering redirect", "a >| clobber",
			[]Segment{{[]string{"a"}, []string{"clobber"}, nil, true, "", false, 0}}},
		{"a redirect with nothing after it names nothing", "echo hi >",
			[]Segment{{[]string{"echo", "hi"}, nil, nil, true, "", false, 0}}},

		// A heredoc delimiter and a here-string's content are not paths.
		{"a heredoc delimiter is not a redirect target",
			"cat <<TAG\nx\nTAG\n> after",
			[]Segment{
				{[]string{"cat"}, nil, nil, true, "", false, 0},
				{nil, []string{"after"}, nil, true, "", false, 1},
			}},
		...
		{"a newline separates two commands", "echo one\necho two",
			[]Segment{
				{[]string{"echo", "one"}, nil, nil, true, "", false, 0},
				{[]string{"echo", "two"}, nil, nil, true, "", false, 1},
			}},
		{"and a comment does not eat it", "echo hi # note\ncat x",
			[]Segment{
				{[]string{"echo", "hi"}, nil, nil, true, "", false, 0},
				{[]string{"cat", "x"}, nil, nil, true, "", false, 1},
			}},

		{"a subshell's parens are separators", "(cd x); echo done",
			[]Segment{
				{[]string{"cd", "x"}, nil, nil, false, "", false, 1},
				{[]string{"echo", "done"}, nil, nil, true, "", false, 3},
			}},
		...
		{"a compound statement keeps its keywords", "until LC_ALL=C grep x f; do :; done",
			[]Segment{
				{[]string{"until", "LC_ALL=C", "grep", "x", "f"}, nil, nil, true, "", false, 0},
				{[]string{"do", ":"}, nil, nil, true, "", false, 1},
				{[]string{"done"}, nil, nil, true, "", false, 2},
			}},
		...
		{"an input redirect is recorded as one", "cat < in",
			[]Segment{{[]string{"cat"}, []string{"in"}, []string{"in"}, true, "", false, 0}}},
		...
		{"a process substitution is a redirect target and not an input",
			"diff <(sort a) <(sort b)",
			[]Segment{
				{[]string{"diff", "sort", "a"}, []string{"("}, nil, false, "", false, 0},
				{[]string{"sort", "b"}, []string{"("}, nil, false, "", false, 1},
			}},
		...
		{"a segment reached through && or || says so",
			...
			[]Segment{
				{[]string{"false"}, nil, nil, true, "", false, 0},
				{[]string{"P=/x"}, nil, nil, true, "&&", false, 1},
				{[]string{"cat", "$P/f"}, nil, nil, true, "||", false, 2},
				{[]string{"cat", "g"}, nil, nil, true, "", false, 3},
			}},
