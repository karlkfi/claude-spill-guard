...
exit 0. `make queue` promotes the four the files in front of you settle; the
`queue` job adds `dangling-link` on a push to `main`, and `make queue
MERGED=true` is the only local command that runs that stronger set.
